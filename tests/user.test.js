...
beforeEach(async () => {
    ...
    .then(
        ...
        err => {console.log("Failed to connect to Mongofrom Jest")}
    )
})
...
describe("For /api/users requests", () => {
    ...
    }, 10000)       // 10000 miliseconds until timeout.
    ...
    // second post of the same user -- we should get an error.
    ...
    describe("Tests For /api/user/{username} requests", () => {
        ...
        it("PATCH for /api/users/{username}", async() => {
            ...
            const res = await request(app).patch('/api/users' + result.username)
            .send({
                name: "new test4",
                surname: "new test4",
                email: "xxx@aueb.gr",
